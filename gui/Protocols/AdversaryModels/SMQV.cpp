...
#define dA H(g1(x),Y,I,R)
#define eA H(Y,g1(x),I,R)
#define dB H(X,g1(y),I,R)
#define eB H(g1(y),X,I,R)

#define sA add(a,mult(dA,x))		// a and x swapped wrt FHMQV
#define baseA mult(B,exp(Y,eA))		// B and Y swapped wrt FHMQV
#define sigmaA exp(baseA,sA)
#define keyA KDF(sigmaA,I,R,g1(x),Y)

#define sB add(b,mult(eB,y))		// b and y swapped wrt FHMQV
#define baseB mult(A,exp(X,dB))		// A and X swapped wrt FHMQV
#define sigmaB exp(baseB,sB)
#define keyB KDF(sigmaB,I,R,X,g1(y))
