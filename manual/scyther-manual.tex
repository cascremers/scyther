...
\setlength{\instdist}{40mm}
\setlength{\markdist}{12mm}
...
Installation instructions are included in the downloadable Scyther
archives. Scyther is available for the Windows,
Linux and Mac OS platforms.

For building from source, including prerequisites and build options,
see the \scr{README.md} file in the project repository.
...
\index{loading files from command line}
Alternatively, you can load a protocol file directly when launching the GUI:
\begin{screen}
python3 scyther-gui.py protocol.spdl
\end{screen}
This will open the specified file immediately in the main window. You can also suppress the splash screen for faster startup:
\begin{screen}
python3 scyther-gui.py --no-splash protocol.spdl
\end{screen}
...
\scr{ns3.spdl} in the Scyther directory (or launch the GUI with \scr{python3 scyther-gui.py ns3.spdl} to load it directly). Your main window should look
...
%............................................................................
\subsection{Visual Notation Reference}
\label{sec:attack-graph-notation}

\index{attack graph!notation}
\index{visual notation}
This section provides a complete reference for understanding the visual elements in attack graphs.

\subsubsection{Arrows and Message Flow}

\index{arrow colors}
\index{message flow}
Arrows in attack graphs represent causality and message dependencies. The color and label of each arrow conveys important information about how the intruder manipulates messages:

\begin{description}
\item[\textcolor{green}{Green arrow (unlabeled)}]
\index{green arrow}
Represents \emph{normal communication}. The message is sent and received in exactly the same form, and both agents agree on who sent the message and who received it. This is ideal protocol behavior with no intruder interference.

\textbf{Example:} Alice sends $\{N_a\}_{pk(Bob)}$ to Bob, and Bob receives exactly that message believing it came from Alice.

\item[\textcolor{orange}{Yellow arrow (\gra{redirect})}]
\index{yellow arrow}
\index{redirect}
Indicates \emph{message redirection}. The message is sent and received in exactly the same form, but the sender and/or receiver beliefs differ from reality. The intruder has redirected the message without modifying its contents.

\textbf{Example:} Alice sends a message to Bob, but the intruder redirects it to Charlie. Charlie receives the exact message but may believe it came from someone else.

\textbf{Why this matters:} Redirection attacks can cause agents to accept messages intended for others, potentially violating authentication properties.

\item[\textcolor{red}{Red arrow (\gra{construct})}]
\index{red arrow}
\index{construct}
Represents \emph{message construction by the intruder}. The received message differs from any sent message. The intruder has constructed a new message using knowledge gained from previous protocol steps.

\textbf{Example:} Alice sends $N_a$, Bob sends $N_b$, and the intruder constructs $(N_a, N_b)$ to send to Charlie.

\textbf{Why this matters:} This indicates active manipulation beyond simple redirection. The intruder is combining, encrypting, or transforming messages.

\item[No incoming arrow]
\index{messages from initial knowledge}
Indicates the received message can be generated entirely from the intruder's \emph{initial knowledge} without waiting for any protocol messages. Common for messages containing only public information like agent names or constants.

\textbf{Example:} A role receives an agent name that could be any value the intruder chooses.
\end{description}

\subsubsection{Event Boxes}

\index{event boxes}
\index{box types}
Different types of events are represented by different box styles:

\begin{description}
\item[Diamond box (run header)]
\index{run header}
\index{diamond box}
Marks the \emph{creation of a protocol run} (role instance). Contains:
\begin{itemize}
\item Run number (e.g., \gra{Run \#0}, \gra{Run \#1})
\item Agent executing the role (e.g., \gra{Agent2 in role I})
\item Agent's beliefs about communication partners (e.g., \gra{I $\rightarrow$ Agent2}, \gra{R $\rightarrow$ Eve})
\item Fresh values generated by this run (e.g., \gra{ni\#0}, \gra{nr\#1})
\item Variable instantiations
\end{itemize}

\item[Rectangular box (send event)]
\index{send event}
\index{send box}
Represents a \emph{send event}. Shows:
\begin{itemize}
\item Event label (e.g., \gra{send\_1})
\item Sender and intended receiver
\item Message content
\end{itemize}

\textbf{Reading tip:} The intended receiver in a send event may differ from where the message actually goes. The intruder always intercepts all messages.

\item[Rectangular box (receive event)]
\index{receive event}
\index{receive box}
Represents a \emph{receive event}. Shows:
\begin{itemize}
\item Event label (e.g., \gra{recv\_2})  
\item Believed sender and actual receiver
\item Message content as understood by the receiver
\end{itemize}

\textbf{Reading tip:} Compare the received message with the sent message at the arrow's source to understand intruder manipulation.

\item[Colored box (claim event)]
\index{claim event}
\index{failed claim}
Represents a \emph{security claim}. Failed claims are typically highlighted (often in red or with special border). The claim box shows:
\begin{itemize}
\item Claim type (\gra{Secret}, \gra{Nisynch}, etc.)
\item Claimed property and parameters
\item Whether the claim succeeded or failed
\end{itemize}

A failed claim in an attack graph indicates \emph{this is the security property that was violated}.
\end{description}

\subsubsection{Agent and Term Notation}

\index{agent notation}
\index{term notation}
\begin{description}
\item[\gra{Agent\#N}]
Represents an agent in the protocol. The number after \gra{\#} is just an identifier to distinguish different agents. Example: \gra{Agent\#0}, \gra{Agent\#1}.

\item[\gra{ni\#N}, \gra{nr\#N}, \gra{nk\#N}]
\index{nonce notation}
Nonces (random values) with their generating run number. \gra{ni\#2} means ``nonce \gra{ni} generated by run \gra{\#2}''. This notation helps track which run created which values.

\item[\gra{\{term\}key}]
\index{encryption notation}
Encryption of \gra{term} with \gra{key}. Examples:
\begin{itemize}
\item \gra{\{Na\}pk(Bob)} - \gra{Na} encrypted with Bob's public key
\item \gra{\{M\}k(A,B)} - \gra{M} encrypted with shared key between A and B
\end{itemize}

\item[\gra{(term1, term2)}]
\index{pairing notation}
Tuple or pairing of multiple terms. Represents concatenation or bundling of message components.
\end{description}

\subsubsection{Reading Attack Graphs: A Strategy}

\index{attack graph!reading strategy}
\index{understanding attacks}
To effectively understand an attack graph:

\begin{enumerate}
\item \textbf{Identify the failed claim:} Look for the highlighted claim event, which is often at the bottom. This is what the attack violates.

\item \textbf{Count the runs:} See how many protocol instances are involved. Most attacks use 2-3 runs.

\item \textbf{Trace backwards from the failed claim:} Follow arrows backwards to see what events led to the security violation.

\item \textbf{Examine arrow colors:}
   \begin{itemize}
   \item Many green arrows? The attack may exploit protocol logic rather than message manipulation.
   \item Red arrows? The intruder is actively constructing messages.
   \item Yellow arrows? The attack relies on message redirection.
   \end{itemize}

\item \textbf{Check agent beliefs:} In run headers, compare who the agent thinks they're talking to versus who is actually in other runs.

\item \textbf{Track nonce origins:} Follow nonce identifiers (e.g., \gra{ni\#2}) to see which run created them and where they're being used.

\item \textbf{Identify the attack step:} Look for the critical point where:
   \begin{itemize}
   \item A message is redirected to the wrong agent (yellow arrow)
   \item The intruder constructs a new message (red arrow)  
   \item An agent accepts a message with wrong beliefs
   \end{itemize}
\end{enumerate}
...
\index{common attacks}
Recognizing these patterns helps quickly understand attack graphs:

\begin{description}
\item[Man-in-the-middle]
\index{man-in-the-middle attack}
Two runs where each agent thinks they're talking to the other, but messages pass through the intruder. Look for: two runs with swapped beliefs, mismatches in the data they should be agreeing on, or multiple red/yellow arrows.

\item[Reflection attack]
\index{reflection attack}
An agent receives back their own message. Look for: a message sent by run N and received by the same run N, often with redirection.

\item[Type confusion]
\index{type confusion attack}
A value meant for one purpose is used for another. Look for: the same nonce or value appearing in different message contexts.

\item[Parallel session attack]
\index{parallel session attack}
Multiple concurrent runs where the intruder mixes messages between sessions. Look for: 3+ runs, messages from different runs being combined.
\end{description}
...
%............................................................................
\section{GUI Settings and Configuration}
\label{sec:gui-settings}

\index{GUI!settings}
\index{settings dialog}
The Scyther GUI provides extensive configuration options through the \scr{Settings} dialog, accessible from the main window. These settings impact the effectiveness and reliability of protocol verification.

\subsection{Verification Parameters}

\index{verification parameters}
The following parameters control how Scyther explores the protocol state space:

\begin{description}
\item[\scr{Maximum number of runs}] 
\index{maximum runs}
Controls how many protocol sessions (runs) Scyther will consider. Default is typically 4-5 runs. 

If a claim is analyzed and the result is ``verified'' or ``falsified'', increasing the maximum number of runs will not change the result, as the claim is provably true, or there exists a counterexample. However, if the result for a claim is ``no attacks within bounds'', then increasing the maximum number of runs may yield different results:
\begin{itemize}
\item May find attacks that require more protocol instances
\item Significantly increases verification time (typically exponentially)
\end{itemize}
Recommended values: 3-4 for quick checks, 5-7 for thorough analysis. However, until ``verified'' or ``falsified'' is reached, results are not conclusive.

\item[\scr{Maximum search depth}] 
\index{maximum search depth}
\index{max-length}
Limits the length of attack traces (number of events). Default is around 20-30. Increase if you suspect attacks with many steps. Very large values can cause memory issues.

\item[\scr{Pruning}]
\index{pruning}
Pruning controls the branch-and-bound search strategy that aims to show the user the ``simplest'' attack, instead of the first attack that it finds. Note that this applies per-claim. Possible values are:
\begin{itemize}
\item 0 = find all attacks
\item 1 = find first attack
\item 2 = find best attack (continues search after finding an attack to see if it can find a simpler one)
\end{itemize}
Default value is 2.

\item[\scr{Maximum number of attacks}]
\index{maximum attacks}
How many attacks to find before stopping. Default is 1 (stop after first attack). Set to higher values or 0 (unlimited) to find all attacks, useful for understanding attack patterns.
\end{description}

\subsection{Verification Modes}

\index{verification modes}
Different verification modes are available through the settings:

\begin{description}
\item[\scr{Standard verification}] Verifies all claims in the protocol file.

\item[\scr{Automatic claims}] Generates and verifies standard security claims automatically for all roles, even if no explicit claims are specified. Note that while this is a quick way to get an overview of potential weaknesses, it may not capture all relevant security properties for a given protocol.

\item[\scr{Characterization}] Generates the complete state-space characterization, showing all possible protocol behaviors within bounds. Users can then manually expect these to investigate if they match expectations.
\end{description}
...
\begin{itemize}
\item Reduce maximum runs (try 3-4 instead of 5+)
\item Set maximum attacks to 1 (stop after finding first attack)
\item Reduce maximum search depth if appropriate
\end{itemize}

For thorough analysis:
\begin{itemize}
\item Increase maximum runs gradually (5, 6, 7...)
\item Set maximum attacks to 0 (find all)
\item Monitor memory usage for large protocols
\end{itemize}

%............................................................................
\section{GUI Menu Reference}
\label{sec:gui-menu}

\index{GUI!menu reference}
This section provides a complete reference for all menu options and GUI features.

\subsection{File Menu}

\index{File menu}
\begin{description}
\item[\scr{New}] Create a new empty protocol file.

\item[\scr{Open}] Open an existing \scr{.spdl} protocol file. Keyboard shortcut: Ctrl+O (Cmd+O on macOS).

\item[\scr{Save}] Save the current file. Keyboard shortcut: Ctrl+S (Cmd+S on macOS).

\item[\scr{Save As}] Save the current file with a new name.

\item[\scr{Exit}] Close the application.
\end{description}

\subsection{Verify Menu}

\index{Verify menu}
\begin{description}
\item[\scr{Verify claims}] 
Verifies all explicit claim statements in the protocol. This is the most common verification mode. The tool will analyze each claim and report whether it is correct or if attacks exist.

\item[\scr{Automatic claims}]
\index{automatic claims}
Automatically generates and verifies standard security properties for all roles in the protocol, even if no explicit claims are specified. Useful for quick security assessment of a protocol.

\item[\scr{Characterize}]
\index{characterization}
Generates a complete state-space characterization showing all possible protocol executions within the specified bounds. This provides a comprehensive view of protocol behavior but can be computationally expensive.
\end{description}

\subsection{Working with Multiple Protocols}

\index{multi-protocol analysis}
To analyze multiple protocols simultaneously:

\begin{enumerate}
\item Create a new \scr{.spdl} file
\item Use \scr{\#include} directives or paste multiple protocol definitions into the same file
\item Run verification on the combined file
\end{enumerate}

This allows detection of multi-protocol attacks where an attacker exploits interactions between different protocols running in the same environment.

\subsection{Exporting Results}

\index{exporting results}
\index{attack graphs!export}
Attack graphs can be saved for inclusion in papers or presentations:

\begin{itemize}
\item Use the system's screenshot functionality to capture attack windows
\item For programmatic access, use the \scr{--dot-output} command-line option (see Section~\ref{sec:cli})
\item The generated \scr{.dot} files can be converted to PDF, PNG, or SVG using Graphviz
\end{itemize}
...
\index{GUI!tips}
\begin{itemize}
\item Use command-line options like the \scr{--no-splash/-N} and \scr{--verify} flags for repeated testing to skip the splash screen, and directly provide input files for verification
\item Save custom settings by modifying the preferences file
\item Use automatic claims first for quick assessment, then add specific claims
\end{itemize}
...
\begin{itemize}
  ...
  \item \scr{Scyther/scyther-mac} (Intel)
  \item \scr{Scyther/scyther-mac-arm} (Apple Silicon)
\end{itemize}
...
\label{sec:python-wrappers}

\index{Python!wrapper scripts}
The Scyther distribution includes several Python wrapper scripts that provide alternative interfaces to the backend tool. These scripts are located in the main Scyther directory.

\subsection{scyther.py: Simple command-line wrapper}
\label{sec:scyther-py}

\index{scyther.py}
The \scr{scyther.py} script provides a simple Python-based command-line interface to the Scyther backend. It accepts the same arguments as the native backend executables.

To use \scr{scyther.py}, run:
\begin{screen}
python3 scyther.py [options] [protocol.spdl]
\end{screen}

For example, to analyze a protocol with specific bounds:
\begin{screen}
python3 scyther.py --max-runs=3 --max-length=10 ns3.spdl
\end{screen}

Running \scr{scyther.py} without arguments displays the help information:
\begin{screen}
python3 scyther.py
\end{screen}

The script automatically detects the appropriate backend binary for your platform (Linux, Windows, or macOS) and forwards all arguments to it. This wrapper is useful for scripting and automation tasks where a consistent interface across platforms is desired.

\subsection{scyther-gui.py: GUI with command-line options}
\label{sec:scyther-gui-cli}

\index{scyther-gui.py!command-line options}
While \scr{scyther-gui.py} is primarily used to launch the graphical interface, it also supports several command-line options for automation and batch processing:

\begin{description}
\item[\scr{-V, --verify}] Immediately verify all claims in the protocol without showing the GUI. Requires an input file.
\begin{screen}
python3 scyther-gui.py --verify protocol.spdl
\end{screen}

\item[\scr{-s, --state-space}] Immediately generate the complete state-space characterization of the protocol without showing the GUI. Requires an input file.
\begin{screen}
python3 scyther-gui.py --state-space protocol.spdl
\end{screen}

\item[\scr{-a, --auto-claims}] Immediately verify the protocol using automatically generated default claims without showing the GUI. Requires an input file.
\begin{screen}
python3 scyther-gui.py --auto-claims protocol.spdl
\end{screen}

\item[\scr{-N, --no-splash}] Suppress the splash screen when launching the GUI. Useful for repeated testing or when the GUI is launched programmatically.
\begin{screen}
python3 scyther-gui.py --no-splash
\end{screen}

\item[\scr{-l, --license}] Display the license information and exit.
\begin{screen}
python3 scyther-gui.py --license
\end{screen}
\end{description}

These options allow \scr{scyther-gui.py} to be used in automated testing scripts while still benefiting from the GUI's result formatting and display capabilities. When used with verification options (\scr{--verify}, \scr{--state-space}, or \scr{--auto-claims}), the results window will appear automatically with the analysis results.

\subsection{json-scyther.py: Batch processing interface}
\label{sec:json-scyther}

\index{json-scyther.py}
\index{batch processing}
The \scr{json-scyther.py} script provides a JSON-based batch processing interface to Scyther. This is particularly useful for running multiple protocol analyses with different configurations or for integrating Scyther into larger automated verification pipelines.

The script reads JSON-formatted input from a file, where each line contains a tuple of:
\begin{enumerate}
\item A list of protocol file paths
\item Scyther options as a string
\item A filter string (typically a claim identifier)
\end{enumerate}

Input format example:
\begin{screen}
[["protocol1.spdl", "protocol2.spdl"], "--max-runs=4", "claim_id"]
\end{screen}

To use \scr{json-scyther.py}:
\begin{screen}
python3 json-scyther.py input.json [line_numbers...]
\end{screen}

If line numbers are specified, only those lines from the input file will be processed. Otherwise, all lines are processed sequentially.

Example usage:
\begin{screen}
python3 json-scyther.py batch-tests.json 1 5 10
\end{screen}

This processes only lines 1, 5, and 10 from \scr{batch-tests.json}.

The JSON interface can be used for:
\begin{itemize}
\item Running systematic parameter sweeps over multiple protocols
\item Integrating Scyther into continuous integration pipelines
\item Automating multi-protocol attack analysis
\item Reproducing research experiments with documented configurations
\end{itemize}
...
This chapter addresses common issues, questions, and best practices for using Scyther effectively.

%............................................................................
\section{Installation and Setup Issues}
\label{sec:trouble-install}

\subsection{macOS: ``Binary cannot be opened because the developer cannot be verified''}

\index{macOS!Gatekeeper}
\index{security warnings}
When downloading the Scyther binary on macOS, the system may block execution due to Gatekeeper security features. 

\textbf{Solution:} Remove the quarantine attribute from the binary:
\begin{screen}
xattr -d com.apple.quarantine Scyther/scyther-mac
\end{screen}

Alternatively, right-click (or Control-click) the binary in Finder and select ``Open'' the first time you run it.

\subsection{GUI fails to start or shows blank windows}

\index{GUI!startup problems}
\index{wxPython}
The Scyther GUI requires wxPython 4.0 or later.

\textbf{Check your Python version and wxPython installation:}
\begin{screen}
python3 --version
python3 -c "import wx; print(wx.version())"
\end{screen}

\textbf{Install or update wxPython:}
\begin{screen}
pip3 install --upgrade wxPython
\end{screen}

On some systems, you may need to use a virtual environment or install additional system dependencies. See the \scr{INSTALL.md} file in the \scr{gui/} directory for platform-specific instructions.

\subsection{Command not found or Python errors}

\index{Python!version requirements}
Scyther requires Python 3. If you see ``command not found'' or syntax errors:

\begin{itemize}
\item Ensure Python 3 is installed: \scr{python3 --version}
\item Use \scr{python3} instead of \scr{python} to run scripts
\item Check that all dependencies are installed: \scr{pip3 install -r requirements.txt}
\end{itemize}

%............................................................................
\section{Understanding Verification Results}
\label{sec:trouble-results}

\subsection{What's the difference between ``No attacks within bounds'' and ``No attacks''?}

\index{no attacks within bounds}
\index{bounded vs unbounded verification}

\begin{description}
\item[\scr{No attacks within bounds}]
Scyther explored the state space up to the configured limits (e.g., maximum 5 runs, maximum depth 20) and found no attacks, but also no proof. Hence, attacks involving more runs or longer traces might still exist.

\textbf{Action:} Gradually increase the maximum number of runs in the settings. Try 6, 7, or 8 runs. If still no attacks are found and the result remains ``within bounds,'' your protocol likely meets the property for practical scenarios.

\item[\scr{No attacks}]
Scyther has proven that no attacks exist on the property in the model, even for unbounded numbers of runs. This is a complete verification result.

\textbf{Meaning:} Your protocol is secure for the specified claims under the Dolev-Yao attacker model and Scyther's semantics.
\end{description}

\subsection{Verification is taking too long}

\index{performance issues}
\index{verification!slow}

If verification doesn't complete in reasonable time:

\begin{enumerate}
\item \textbf{Reduce maximum runs:} Try 3 or 4 instead of 5+. Most attacks are found with few runs.

\item \textbf{Limit attack search:} Set maximum attacks to 1 (stop after finding first attack).

\item \textbf{Check protocol complexity:} Protocols with many roles, long message sequences, or complex terms take longer to verify.

\item \textbf{Try automatic claims first:} If you have many explicit claims, try verifying with automatic claims to get quick feedback.
\end{enumerate}

\subsection{Why did Scyther find an attack I don't think is realistic?}

\index{unrealistic attacks}
\index{Dolev-Yao model}

Scyther uses the Dolev-Yao attacker model, which makes certain assumptions:

\begin{itemize}
\item \textbf{Perfect cryptography:} Encryption cannot be broken without the key
\item \textbf{Attacker controls the network:} The attacker can intercept, modify, and inject messages
\item \textbf{Nonces are perfectly random:} Cannot be guessed by the attacker
\end{itemize}

If an attack seems unrealistic:

\begin{enumerate}
\item Check if it relies on assumptions not in your threat model
\item Consider if additional checks or type distinctions should be added to the protocol
\item Verify that your protocol specification matches the intended implementation
\item Remember: Even ``unrealistic'' attacks may reveal design weaknesses worth addressing
\end{enumerate}

%............................................................................
\section{Protocol Modeling Issues}
\label{sec:trouble-modeling}

\subsection{No attacks found but I expected one}

\index{modeling errors}
\index{false negatives}

Common causes:

\begin{enumerate}
\item \textbf{Modeling errors:} Review your protocol specification for syntax errors, incorrect term construction, or missing messages. Try automatic claims and in particular reachability claims: if a reachability claim is not satisfied, there is likely a modeling error.

\item \textbf{Insufficient runs:} The attack requires more protocol instances than your bound. Increase maximum runs to 6-8.

\item \textbf{Wrong claim type:} Make sure you're using the right claim type (e.g., \spd{Secret} vs \spd{Nisynch}).

\item \textbf{Over-constrained roles:} Check that role variables aren't unnecessarily constrained. The attacker should be able to execute roles with different agents.

\end{enumerate}

\subsection{Parse errors and syntax issues}
...
\begin{itemize}
\item Missing semicolons after statements
\item Mismatched braces \{ \} or parentheses ( )
\item Undefined variables (all variables must be declared as \spd{var} or \spd{fresh})
\item Wrong term construction (check encryption syntax: \spd{\{term\}key})
\item Case sensitivity (Scyther is case-sensitive)
\item Comments not properly closed (\spd{/* ... */})
\end{itemize}

\textbf{Tip:} Look at example protocols in the \scr{Protocols/} directory for correct syntax patterns.

\subsection{Unexpected variable bindings}

\index{variable binding}
\index{role matching}

Variables in roles can be bound in ways you might not expect:

\begin{itemize}
\item A role can run with any agent, including the intruder
\item Variables are bound when a message is received
\item The same agent can play multiple roles (including communicating with themselves)
\end{itemize}

To constrain role execution, use match statements or add inequalities in global declarations.

%............................................................................
\section{Common Questions}
\label{sec:faq-common}

\subsection{Should I use automatic claims or write my own?}

\index{automatic claims vs explicit claims}

\textbf{Use automatic claims when:}
\begin{itemize}
\item Getting started with a new protocol
\item Doing quick security assessment
\item You want standard authentication and secrecy properties
\end{itemize}

\textbf{Write explicit claims when:}
\begin{itemize}
\item You need specific, non-standard properties
\item You want fine-grained control over what's verified
\item You're analyzing a variant and want to compare specific claims
\item Writing formal proofs that reference specific claims
\end{itemize}

\textbf{Best practice:} Use automatic claims first to understand the protocol, then add explicit claims for specific properties you care about.

\subsection{Can Scyther verify protocols with Diffie-Hellman or other cryptographic operations?}

\index{Diffie-Hellman}
\index{cryptographic operations}
\index{equational theories}

Scyther supports standard Dolev-Yao operations (encryption, hashing, pairing). For advanced cryptographic operations like Diffie-Hellman, XOR, or exclusive-or more advanced tools like the Tamarin prover are more appropriate. 

However, you can approximate some of these operations in Scyther using helper protocols (see Section~\ref{sec:advanced}).

\subsection{How do I verify a protocol with more than two roles?}

\index{multi-role protocols}
\index{three-party protocols}

Scyther handles protocols with any number of roles. Simply define all roles within the same protocol:

\begin{spdl}
protocol ThreeParty(Alice,Bob,Server)
{
  role Alice { ... }
  role Bob { ... }
  role Server { ... }
...
Each role can have its own claims. Scyther will consider all possible interleavings of the roles.


\subsection{Where can I find more example protocols?}

\index{example protocols}
\index{protocol library}

Example protocols are in the \scr{Protocols/} directory:

\begin{itemize}
\item \scr{Protocols/} - Standard protocols (Needham-Schroeder, Kerberos, TLS, etc.)
\item \scr{gui/Protocols/} - Additional protocol examples
\item The examples cover various patterns: challenge-response, key exchange, authenticated key establishment
\end{itemize}

Study these examples to learn modeling patterns and best practices.
