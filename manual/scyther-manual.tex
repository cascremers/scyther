...
Thick vertical downward arrows typically indicate the flow of time and the sequence of events within a protocol run. Horizontal and diagonal arrows represent message transmission between agents. 
...
\item[Orange oval (intruder event)]
\index{intruder event}
\index{orange oval}
Represents an \emph{intruder action}, for example:
\begin{itemize}
  \item Establishing the terms that the intruder initially knows, such as constants, agent names, and intruder-generated nonces (\gra{Initial intruder knowledge})
  \item Constructing a new message, e.g., \gra{encrypt} or \gra{decrypt}
  \item faking a message, e.g., \gra{fake sender Eve}
\end{itemize}
