...
\index{message flow}
\index{arrow colors}
Arrows in attack graphs represent causality and message dependencies:

\begin{description}
\item[Bold green arrows]
\index{green arrows}
Indicate \emph{exact communication} where a message is sent and received exactly as specified in the protocol, without intruder interference or modification. These are drawn in bold with color \gra{forestgreen}.

\item[Black arrows]
\index{black arrows}
All other arrows (default style). This includes:
\begin{itemize}
\item Messages intercepted by the intruder
\item Messages from the intruder to agents
\item Messages between intruder operations
\item Dependencies between events
\end{itemize}
\end{description}

Thick vertical downward arrows typically indicate the flow of time and the sequence of events within a protocol run. Horizontal and diagonal arrows represent message transmission or dependencies. When a message path goes through an orange node (see below), the message has been modified or redirected by the intruder. 
...
\begin{description}
\item[Rectangular box (run header)]
\index{run header}
...
\item[Orange node (intruder operation or redirect)]
\index{intruder event}
\index{orange node}
\index{redirect node}
Represents an \emph{intruder action}. These nodes are filled with an orange color. They include:
\begin{itemize}
  \item \textbf{Initial knowledge:} Shown as \gra{M} or similar, representing terms the intruder knows from the start (agent names, constants, compromised keys)
  \item \textbf{Construction operations:} Labeled \gra{encrypt}, \gra{decrypt}, or \gra{apply}, showing cryptographic operations the intruder performs
  \item \textbf{Redirect nodes:} When a message is modified between send and receive, an intermediate orange node appears with labels like:
  \begin{itemize}
    \item \gra{select} -- extracting a subterm from a message
    \item \gra{redirect to} -- changing the intended recipient
    \item \gra{fake sender} -- changing the apparent sender
    \item \gra{combine} -- other modifications
  \end{itemize}
\end{itemize}
...
\index{claim event}
\index{failed claim}
Represents a \emph{security claim}. The claim box shows:
\begin{itemize}
...
\item \textbf{Examine arrows and nodes:}
   \begin{itemize}
   \item Many green arrows? The attack may exploit protocol logic rather than message manipulation.
   \item Black arrows through orange nodes? The intruder is modifying or redirecting messages.
   \item Orange \gra{encrypt}/\gra{decrypt} nodes? The intruder is actively constructing new messages.
   \end{itemize}
...
\item \textbf{Identify the attack step:} Look for the critical point where:
   \begin{itemize}
   \item A message is redirected or modified (orange redirect node in the arrow path)
   \item The intruder constructs a new message (orange \gra{encrypt}/\gra{decrypt} node)  
...
Two runs where each agent thinks they're talking to the other, but messages pass through the intruder. Look for: two runs with swapped beliefs, mismatches in the data they should be agreeing on, or multiple orange redirect nodes.
